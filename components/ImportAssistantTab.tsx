...
export function ImportAssistantTab({ toast, onFillFeedback, onFillHomework, onNavigate }: Props) {
    ...
    const handler = (e: ClipboardEvent) => {
      ...
      for (let i = 0; i < items.length; i++) {
        if (items[i].type.startsWith('image/')) {
          ...
          if (!file) return;
          if (!['image/png', 'image/jpeg', 'image/jpg'].includes(file.type)) { toast('仅支持 PNG/JPG 图片'); return; }
  ...
  const handleParse = useCallback(() => {
    if (!text.trim()) { toast('请先粘贴报告内容'); return; }
    setParsed(parseImportedText(text.trim()));
    toast('解析完成');
  }, [text, toast]);

  const handleFillFeedback = useCallback(() => {
    if (!parsed) return;
    onFillFeedback(buildStructuredDataFromImport(parsed), parsed.supplement);
  ...
  return (
    <div>
      <div className="card">
        ...
        <div style={{ fontSize: '.84rem', color: 'var(--muted)', marginBottom: 16 }}>粘贴上课报告文字和作业链接，工具自动整理后填入课后反馈和正课作业。</div>
        <div className="field">
          <label>上传截图 <span className="hint">（选填，预览参考）</span></label>
          <div className="drop-zone" onClick={() => fileInputRef.current?.click()} onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); const f = e.dataTransfer.files?.[0]; if (f) handleFile(f); }}>
            {imagePreview ? (
              <div className="drop-zone-preview"><img src={imagePreview} alt="预览" /><button className="drop-zone-delete" onClick={(e) => { e.stopPropagation(); setImagePreview(''); }}>删除</button></div>
            ) : (<><div className="drop-zone-icon">📁</div><div className="drop-zone-text">点击上传、拖拽图片，或直接粘贴截图</div><div className="drop-zone-hint">仅作预览，不会自动识别文字</div></>)}
          </div>
        ...
        <div className="field">
          ...
          <textarea placeholder={`学生：睿泽\n日期：5月24日\n词库：小学短语体验词库\n今日共识记词汇：70个\n学新词汇：23个\n\nhttps://test.hzdmsyy.com/pc/prints?id=247343`} value={text} onChange={(e) => setText(e.target.value)} style={{ minHeight: 140 }} />
        </div>
      ...
      {parsed && (
        <div className="card">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px 20px', fontSize: '.84rem', marginBottom: 12 }}>
